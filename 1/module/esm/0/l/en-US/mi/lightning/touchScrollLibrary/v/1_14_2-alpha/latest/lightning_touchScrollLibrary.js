...
export class TouchScroller {
  ...
  handleTouchMove(event) {
    ...
    var target = event.target; // Elements that have their own scroll behavior such as input type range never cause
    ...
    if (canScrollTarget || canScrollCurrent) {
      const curY = getPositionY(event); // Determine if the user is trying to scroll past the top or bottom.

      let atTop = this._startY < curY && isAtTop(el);
      let atBottom = this._startY > curY && isAtBottom(el); // When target is scrollable, we also need to check if the user is trying to scroll
...
_registerDecorators(TouchScroller, {
